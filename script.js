...
window.closePlayer = async function() {
  if (isLandscape) await toggleLandscape();
  const overlay = document.getElementById('playerOverlay');
  const video = document.getElementById('playerVideo');
  const iframe = document.getElementById('drivePlayer');
...
window.toggleLandscape = async function() {
  const overlay = document.getElementById('playerOverlay');
  const btn = document.querySelector('.player-fs-btn');
  const iframe = document.getElementById('drivePlayer');
  const isDrive = overlay.classList.contains('drive-mode') && iframe && iframe.src;
  ...
  try {
    if (!isLandscape) {
      ...
      // Khusus Google Drive: fullscreen langsung iframe-nya.
      // Ini mengurangi tampilan berantakan karena header web tidak ikut numpuk.
      if (isDrive && iframe.requestFullscreen && !document.fullscreenElement) {
        await iframe.requestFullscreen();
      } else {
        overlay.classList.add('fake-fs');
      ...
      overlay.classList.remove('fake-fs');
      isLandscape = false;
    ...
    // Kalau fullscreen iframe ditolak browser, pakai fullscreen CSS bersih.
    overlay.classList.add('fake-fs');
    isLandscape = true;
    if (btn) btn.classList.add('active');
  }
